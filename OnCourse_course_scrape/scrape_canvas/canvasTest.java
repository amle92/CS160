            ...
            for (int j = 0; j < 10; j++) {
                ...
                try {
                    ...
                    dStrDate = sdf.parse(ddyy[1] + "/" + month + "/" + ddyy[3]);
                     int year = (Integer.parseInt(ddyy[1]));
                     int day = (Integer.parseInt(ddyy[3]));
                ...
                try {
                    ...
                    //String realStrMonth = tempEndDate.substring((tempEndDate.indexOf("-") + 1),
                      //                    (tempEndDate.indexOf(" ", tempEndDate.indexOf(" ") + 1)));
                    String[] realStrMonth = realEndDate.split(" ");
                    int month = month(realStrMonth[1]);
                    System.out.println("Real End:" + realStrMonth[1]);
                    System.out.println(month);
                    System.out.println(realStrMonth[0]);
                    String toParseDate = realEndDate.replace(","," ");
                    ...
                    System.out.println("Start End Month:" + month);
                    System.out.println("Start End Day:" + day);
                    System.out.println("Start End Year:" + year);
                    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
                    endDate = (Date) sdf.parse(ddyy[2] + "/" + month + "/" + ddyy[4]);
                    end = new DateTime(year, month, day, 0, 0, 0, 0);
                    //long dateDiff = dEndDate.getTime() - dStrDate.getTime();
                    //crsduration = (int) TimeUnit.DAYS.convert(dateDiff, TimeUnit.MILLISECONDS);
                    //System.out.println("ddyy" + dEndDate.toString());
               ...
               // System.out.println("Course Duration" + crsduration);
                /** Certificate **/
