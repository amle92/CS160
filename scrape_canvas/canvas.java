...
public class canvas {
    ...
    public final String DEFAULT_DATE = "2020-";
    /**
     * @param args
     * @throws IOException
    ...
    public static void main(String[] args) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, ParseException, JSONException {
        ...
        for (int j = 0; j < link.size(); j++) {
            ...
            try {
                ...
                System.out.println("TempStrDate is: " + tempStrDate);

                // Handles if start date begins with "Available beginning..."
                if (tempStrDate.contains("Available")){
                    String toParseDate = tempStrDate.replace(",", "");
                    String cleanStart = tempStrDate.replace("beginning " , "");
                    String[] startArray = cleanStart.split(" ");
                    realStrDate = startArray[1];
                    System.out.println(" Available beginning... "  + realStrDate);
                    int month = month(realStrDate);
                    System.out.println(" Available beginning... "  + month);
                    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
                    dStrDate = sdf.parse(startArray[3] + "/" + month + "/" + startArray[4]);
                    System.out.println("Available SQL date" + startArray[3] + "/" + month + "/" + startArray[4]);
                    sqlStrDate = new Date(dStrDate.getTime());
                }
                else
                // Start date begins with "Starts... "
                realStrDate = tempStrDate.substring(7, tempStrDate.indexOf(" ", tempStrDate.indexOf(" ") + 1));
                System.out.println("Starts with ... " + realStrDate);
                String toParseDate = tempStrDate.replace(","," ");
                int month = month(realStrDate);
